...
// Populate sessions after program is published.
const sessions: { title: string; authors: string[]; affiliations: string[]; topics: string[] }[] = [];

async function seedSessions() {
  if (sessions.length === 0) {
    console.log("No ATC sessions to seed — awaiting program announcement");
    return;
  }
  ...
  if (newSessions.length === 0) {
    console.log("All ATC sessions already exist");
    return;
  }
