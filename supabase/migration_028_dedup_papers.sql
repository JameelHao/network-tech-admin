-- 1. Clean existing duplicates first (merge paper_topics, delete rest)
WITH dup_groups AS (
  SELECT
...
-- 2. Now add unique index (no duplicates left)
CREATE UNIQUE INDEX IF NOT EXISTS idx_papers_norm_title ON papers (lower(regexp_replace(trim(title), '\s+', ' ', 'g')));
