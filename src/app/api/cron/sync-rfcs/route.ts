...
export async function GET() {
  ...
  const items: { rfc_number: number; title: string; link: string; description: string; pubDate: string }[] = [];
  ...
  while ((match = itemRegex.exec(xml)) !== null) {
    ...
    items.push({
      rfc_number: parseInt(numMatch[1], 10),
      title,
      link: cleanHTML(extractTag(raw, "link")),
      description: cleanHTML(extractTag(raw, "description")),
      pubDate: extractTag(raw, "pubDate") || new Date().toISOString(),
    });
  }
  ...
  const result = await upsertRfcs(parsed);

  const supabase = await createClient();
  const now = new Date().toISOString();
  await supabase.from("sync_meta").upsert(
    { entity: "rfcs", last_sync_at: now, last_result: { inserted: result.inserted, updated: result.updated, total: parsed.length } },
    { onConflict: "entity" },
  );

  return NextResponse.json({ success: true, ...result, total: parsed.length });
}
