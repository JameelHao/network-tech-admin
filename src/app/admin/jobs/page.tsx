...
import { computeJobStats } from "@/lib/admin/jobs-utils";
...
export default async function JobsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const { thisWeek, active, sourceCount } = computeJobStats(items);
  ...
  return (
    <>
      ...
      <main className="px-6 xl:px-10 py-10">
        ...
        <SyncStatusBar entity="jobs" lang={lang} labels={{ lastSync: t.sync.lastSync, refresh: t.sync.refresh, refreshing: t.sync.refreshing, noData: t.sync.noData, syncResult: t.sync.syncResult, sourcesFailed: t.sync.sourcesFailed }} />
        ...
        <section className="mb-4 rounded-lg border border-line bg-surface overflow-hidden">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-line">
            <Stat label={t.jobs.totalJobs} value={result.total} sub={t.jobs.totalSub} />
            <Stat label={t.jobs.thisWeek} value={thisWeek} sub={t.jobs.thisWeekSub} />
            <Stat label={t.jobs.activeJobs} value={active} sub={t.jobs.activeSub} />
            <Stat label={t.jobs.sourceCount} value={sourceCount} sub={t.jobs.sourceCountSub} />
          </div>
        </section>
