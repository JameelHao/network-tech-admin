...
export default async function CompaniesPage() {
  ...
  const [paperCounts, newsCounts, repoStats] = await Promise.all([
  ...
  const companies = Object.entries(COMPANY_NAMES).map(([slug, name]) => {
    ...
    return { slug, name, color: COMPANY_COLORS[slug] ?? "", paperCount, newsCount, repoCount, latestRepoPush, total: paperCount + newsCount + repoCount };
  });
