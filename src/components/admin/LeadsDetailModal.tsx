  ...
  return (
    <DetailModal title={lead.title} label={t.nav.leads} onClose={onClose}>
      ...
      <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-[13px]">
        ...
        <div><dt className="font-mono text-[10px] uppercase tracking-[0.18em] text-ink-500">{t.list.updatedAt ?? "Updated At"}</dt><dd className="mt-1 text-ink-800 tabular-nums">{lead.updated_at?.slice(0, 10) ?? "—"}</dd></div>
      </dl>
      {lead.summary && <div><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-ink-500 mb-1">{t.detail.summary ?? "Summary"}</p><p className="text-[13px] text-ink-600 leading-relaxed">{lead.summary}</p></div>}
