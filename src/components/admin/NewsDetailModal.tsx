...
export function NewsDetailModal({ item, t, lang, onClose }: { item: NewsItem; t: Dict; lang: Lang; onClose: () => void }) {
  return (
    <DetailModal title="News" label={t.nav.news} onClose={onClose}>
      ...
      <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-[13px]">
        ...
        <div><dt className="font-mono text-[10px] uppercase tracking-[0.18em] text-ink-500">{t.list.publishedAt}</dt><dd className="mt-1 text-ink-800 tabular-nums font-mono text-[12px]">{item.pub_date ? relativeTime(item.pub_date, lang) : "—"}</dd></div>
